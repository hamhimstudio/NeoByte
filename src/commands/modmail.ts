...
export class ModMailCommands {
    static generateEmbed() {
        ...
        embed.setDescription(
            `Welcome to our modmail system! Please select the reason behind creating this inquiry and a  inquiry channel will be created!`
        )
    ...
    async claim(interaction: CommandInteraction): Promise<void> {
        ...
        interaction.editReply(
            `Inquiry claimed by ${interaction.user.toString()}`
        )
    }
    ...
    async close(interaction: CommandInteraction): Promise<void> {
        if (
            ...
            await interaction.reply(
                'This command can only be used in a valid text channel!'
            )
            return
        }
    ...
    async open(interaction: CommandInteraction): Promise<void> {
        ...
        await interaction.channel?.send(
            `This inquiry has been reopened! By ${interaction.user.toString()} If you wish close it again, please use the  \`/mail close\` slash command!`
        )
    }
    ...
    async removeMail(interaction: ButtonInteraction): Promise<void> {
        if (
            ...
            return
        if (
            !interaction.member.roles.cache.has(process.env.MODMAIL_PING_ROLE!)
        ) {
            await interaction.reply({
        ...
        if (interaction.channel) {
            ...
            await user.send({
                content:
                    'Your inquiry has been deleted and archived.\nHere is the transcript!',
                files: [attachment],
            })
            ...
            if (log_channel instanceof TextChannel)
                await log_channel.send({
                    content:
                        'An inquiry has been deleted and archived.\nHere is the transcript!',
                    files: [attachment],
                })
