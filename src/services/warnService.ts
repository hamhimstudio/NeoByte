...
export default class warnService {
    ...
    async findWarning(id: string) {
        return await this.warnRepository.findOne({
    ...
    async createWarning(
        ...
        return await this.warnRepository.createWarning(
            userId,
            guildId,
            reason,
            moderatorUserId,
    ...
    async getTotalPoints( userId: string, guildId: string) {
        let totalPoints = 0
        const warnings = await this.getWarningForUser(userId, guildId)
